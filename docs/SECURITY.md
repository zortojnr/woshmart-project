...
- **Encryption in transit:** TLS everywhere — Twilio↔backend, backend↔database, backend↔Redis (where supported by the provider), Retool↔Admin API.
  - **Known, deliberate exception — staging Redis only:** staging's Redis runs on Redis Cloud's free tier, chosen specifically because Render allows only one free Key Value instance per workspace (already used by production) and Upstash allows only one free database per account without adding a payment method (already used by dev) — see `docs/SETUP_GUIDE.md` §2's Redis row. Redis Cloud's free tier does not support TLS at all, so staging's `REDIS_URL` is a plain `redis://` connection, unencrypted in transit. Accepted deliberately because staging only ever holds test data (the tester's own phone numbers, never real customers) and what actually transits this connection is limited to BullMQ job pointers (`{phoneNumber}`/`{orderId}`) and rate-limit counters, not order/payment content. **This does not extend to production.** Production's Redis (Render Key Value) already supports TLS and Render's private network (same-region, same-account), and must keep using both — this exception is scoped to staging specifically and is not precedent for relaxing production's transport encryption.
