...
| Database | **Neon** for dev (already set up, free tier); **Render Postgres** for staging; **Supabase** (free tier) for production — production moved off Render Postgres mid-Phase-8 after weighing the tradeoffs live: Supabase's free tier pauses rather than permanently deletes on inactivity, is one-click restorable, needs no card, and a daily heartbeat avoids the pause entirely | Production currently has **no backup/disaster-recovery plan** of its own (unlike staging's `pg_dump` → B2 pipeline, `docs/SECURITY.md` §3.9) — a real open item, tracked in `docs/BUILD_LOG.md`'s Post-MVP log, needed before real customer/payment data accumulates |
| Redis | **Upstash** for dev, **Redis Cloud** (Redis Inc.'s own managed offering) for staging, **Render Key Value** for production — three different providers, deliberately, not arbitrary sprawl | Each free-tier limit ruled out reusing a provider: Render allows only one active free Key Value instance per workspace (blocks Render for both staging and production), and Upstash allows only one free database per account without adding a payment method (blocks a second Upstash database for staging, since dev already holds the one free slot). Redis Cloud's free tier (30MB, 30 connections, 100 ops/sec) needs no card and is a separate account from Upstash entirely, comfortably enough for staging's low, manual-test-driven traffic. Small footprint everywhere, don't over-provision. |
...
| Staging | Render Postgres (separate instance) | **Redis Cloud** (separate provider from dev's Upstash and production's Render Key Value, free tier — see the Redis row above for why staging landed on a third provider rather than reusing either) | Dedicated staging sender (never the production number) | Pre-release verification, safe template testing, Phases 1–8 |
| Production | **Supabase** (free tier — see the Database row above for why) | Render Key Value (separate instance, free tier) | Production business number | Real customers, Phase 8a onward only |
