...
| Database | **Neon** for dev (already set up, free tier); **Render Postgres** for staging; **Supabase** (free tier) for production — production moved off Render Postgres mid-Phase-8 after weighing the tradeoffs live: Supabase's free tier pauses rather than permanently deletes on inactivity, is one-click restorable, and needs no card | Production has a daily `pg_dump` → Backblaze B2 pipeline (`docs/SECURITY.md` §3.9, `.github/workflows/production-backup.yml`), 30-day retention. That same daily job doubles as the anti-pause heartbeat — no separate keep-alive mechanism needed, since it's a real query running well within Supabase's ~7-day inactivity threshold |
